...
export default function UniversalResultScreen({ route }: UniversalResultScreenProps) {
  ...
  const saveToGallery = async () => {
    try {
      ...
      trackEvent('action', { type: 'save_failed', mode, error: (error as Error)?.message || 'unknown' });
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const shareImage = async () => {
    try {
      ...
      trackEvent('action', { type: 'share_failed', mode, error: (error as Error)?.message || 'unknown' });
    }
  };
