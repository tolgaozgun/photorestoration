...
class MenuService {
  ...
  async getDevelopmentMenuConfig(environment: string = 'production'): Promise<MenuConfigResult | null> {
    try {
  ...
  async checkForUpdates(currentVersion: string, environment: string = 'production'): Promise<UpdateInfo> {
    try {
      const headers = await this.getAuthHeaders();
      const response = await axios.get(
