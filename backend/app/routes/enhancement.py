...
async def enhance_image(
    ...
    try:
        ...
        if not UserService.has_credits(user, resolution):
            logger.warning(f"User {user_id} has no credits available for {resolution} enhancement.")
        ...
        try:
            ...
            UserService.deduct_credits(user, resolution)
            ...
            try:
                ...
                UserService.refund_credits(user, resolution)
                db.commit()
            ...
            return EnhancementResponse(
                ...
                watermark=watermark,
                processing_time=processing_time,
                remaining_credits=(user.hd_credits if resolution.lower() == "hd" else user.standard_credits),
                remaining_today=(daily_limits["remaining_today_hd"] if resolution.lower() == "hd" else daily_limits["remaining_today_standard"])
            )
